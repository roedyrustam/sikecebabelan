...
class Galery extends CI_Controller
{
    ...
    public function profil_kecamatan()
    {
        ...
        $this->load->view('galery/profil_kecamatan',$data);
    ...
    public function edit_profil()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect('galery/profil_kecamatan');
        }
    }

    public function s_kecamatan()
    {
        $data['profil'] = $this->galery->profil();
        $judul = [
            'title' => 'Galery',
            'sub_title' => 'Struktur kecamatan'
        ];

        // $data['sm'] = $this->db->get('surat_masuk')->row_array();
        // var_dump($data);
        $this->load->view('templates/header', $judul);
        $this->load->view('galery/s_kecamatan',$data);
        $this->load->view('templates/footer');
    }

    public function edit_s_kecamatan()
    {
        $this->form_validation->set_rules('s_kecamatan', 'Struktur kecamatan', 'trim');
        ...
        if ($this->form_validation->run() == false) {
            ...
            // var_dump($data);
            $this->load->view('templates/header', $judul);
            $this->load->view('galery/edit_s_kecamatan',$data);
            $this->load->view('templates/footer');
        }else{
            $id = $this->uri->segment(3);
            $upload_sk = $_FILES['s_kecamatan']['name'];
            ...
            if ($upload_sk) {
                ...
                if ($this->upload->do_upload('s_kecamatan')) {
                    $old_sk = $data['galery']['s_kecamatan'];
                    unlink(FCPATH . 'assets/galery/' . $old_sk);

                    $s_kecamatan = $this->upload->data('file_name');
                } else {
                    echo $this->upload->display_errors();
                }
            }else {
                $s_kecamatan = $this->input->post('s_kecamatan_old');
            }
            // var_dump($s_kecamatan);
            // die;
            
            $this->galery->UpdateSkecamatan($s_kecamatan,$id);
            $this->session->set_flashdata('success', 'Berhasil Di Update!');
            redirect('galery/s_kecamatan');
        }
    ...
    public function s_pemuda()
    {
        ...
        $judul = [
            'title' => 'Galery',
            'sub_title' => 'Struktur Pemuda kecamatan'
        ];
    ...
    {
        $this->form_validation->set_rules('s_pemuda', 'Struktur Pemuda kecamatan', 'trim');
